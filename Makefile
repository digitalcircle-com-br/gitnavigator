...
build_windows_amd64:
	GOOS=windows GOARCH=amd64 go build -ldflags="-s -w -H=windowsgui" -o packing/windows_amd64/gitnavigator.exe .
...
build_darwin_arm64:
	GOOS=darwin GOARCH=arm64 go build  -ldflags="-s -w" -o packing/_mac/GitNavigator.app/Contents/MacOS/gitnavigator cmd/main.go
	rm -f packing/distro/devserver_darwin_arm.zip
	cd packing/_mac && zip  -r ../../packing/distro/gitnavigator_darwin_arm64.zip GitNavigator.app
	rm packing/_mac/DevServer.app/Contents/MacOS/gitnavigator	
